object caseclasses {
  ...
  case class Money(val amount: Int = 1, val currency: String = "USD") { // Default values have been defined in the argument. If no value is assigned then those default variables will be considered.
    //define a function called '+' to add two money objects
    def +(other: Money): Money = Money(amount + other.amount)  // Here '+' symbol overrides the default operation.
  }
  // In the below val statements notice that we are not using the new keyword anywhere.
